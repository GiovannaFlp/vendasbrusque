...
export function EditListingForm({ listing, categories }: EditListingFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(`/api/anuncios/${listing.id}`, {
        method: 'PATCH',
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="card p-5">
        ...
        <div className="grid grid-cols-4 gap-3">
          ...
          {newPreviews.map((src, i) => (
            <div key={`new-${i}`} className="relative aspect-square rounded-lg overflow-hidden group bg-gray-100">
              <Image src={src} alt="" fill className="object-cover" />
              <button
                type="button"
                onClick={() => removeNew(i)}
      ...
      <div className="card p-5 space-y-4">
        ...
        <div>
          ...
          <textarea name="description" value={form.description} onChange={handleChange} required rows={5} className="input-field resize-none" />
        </div>
      </div>
      ...
      <div className="card p-5 space-y-4">
          ...
          {[
            ...
            <button
              ...
              className={`p-3 rounded-lg border text-sm font-medium transition-colors ${
                form.priceType === opt.value ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-200 text-gray-600'
              }`}
            >
              {opt.label}
            </button>
        ...
        {(form.priceType === 'fixed' || form.priceType === 'negotiable') && (
          <div>
            ...
            <input type="number" name="price" min="0" step="0.01" value={form.price} onChange={handleChange} className="input-field" />
          </div>
        )}
      </div>
